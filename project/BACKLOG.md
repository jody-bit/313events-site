...
**EPIC-006 note:** discovery approved with refinements 2026-09-21 (Product Owner review of the "Metadata Self-Healing" discovery request). **SH.1** (venue address/city repair from canonical data) is `IN PROGRESS` — the epic's own recommended first implementation, additive-only and provenance-independent. SH.4, SH.3, SH.6, SH.2, SH.5, SH.7, and SH.N remain `BACKLOG`, ordered but not scoped to `READY`; see the epic file for the locked repair-priority ordering (canonical data → already-fetched-but-discarded data → cross-source → generated → human) and why generated descriptions (SH.2) were deliberately moved behind provenance (SH.6).
