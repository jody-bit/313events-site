...
// "RESOLVE EMBEDDED FUTURE EVENTS" (2026-09-25) — the real C&G Newspapers
// article "Photos sought for exhibit celebrating America's 250th
// birthday," captured live while tracing this fix. Jody's own override of
// an earlier "not a fit" judgment: "The article explicitly contains a
// future public event, 'Backyard and Beyond,' opening Nov. 9 at Madison
// Heights Public Library, 240 W. 13 Mile Road." Verbatim (trimmed) body
// text — a real, deliberately hard case for this pipeline: the article's
// own primary framing is a submission call (a photo deadline, Oct. 10,
// mentioned THREE times), not the event itself, and the same article also
// references a completely different, already-running exhibit (the Tiny
// Art Show) that happens to end on that exact same Oct. 10 date.
const BACKYARD_HEADLINE = "Photos sought for exhibit celebrating America's 250th birthday";
const BACKYARD_BODY =
  "MADISON HEIGHTS — An upcoming exhibit aims to bring Americans together by letting them share pictures of " +
  "places they love, whether across the state or country, or even at home. The Madison Heights Public Library " +
  "is partnering with the Madison Heights Arts Board on the display, titled \"Backyard and Beyond.\" Key " +
  "organizers include librarians Heather Hames and Rae Stoloff, and Arts Board members Laurie Geralds and " +
  "Sandie Young. Librarians Rae Stoloff, left, and Heather Hames sort through photo submissions for the " +
  "\"Backyard and Beyond\" exhibit celebrating America's 250th birthday, to be displayed starting in November " +
  "at the Madison Heights Public Library. The deadline for submissions is Oct. 10. Photo by Liz Carnegie The " +
  "exhibit goes live on Monday, Nov. 9, inside the library at 240 W. 13 Mile Road, but the deadline for " +
  "submissions is Saturday, Oct. 10. Photos must be original works. Minor editing such as cropping, lighting " +
  "or color correction is allowed, but heavily altered images are not. The exhibit also won't accept images " +
  "generated by AI. Submissions should be high resolution with no watermarks, borders or added text. Photos " +
  "can be submitted online at madisonheightsmi.gov. Type \"backyard beyond\" in the search bar and choose the " +
  "first option to find the form. Alternatively, go to bit.ly/mhj250photo. Physical photos can also be " +
  "submitted by bringing them to the library and requesting that digital copies be made with the photo " +
  "copier. Call the library at (248) 588-7763 for more information. As for the content, the pictures can be " +
  "set in places near and far, so long as they're in the United States. It could be a quiet moment in your " +
  "backyard garden, a sunset at a park, a favorite camping spot up north, a view of mountains in another " +
  "state — all are eligible. Ultimately, a subcommittee set up by the library and Arts Board will have the " +
  "final say on which photos make the cut. Of course, the photos must be appropriate for people of all ages. " +
  "Hames said the project was inspired by another exhibit currently on display at the library: the Tiny Art " +
  "Show. Over the summer, residents could collect blank 4-by-4-inch canvases at the library and draw or paint " +
  "on them as they please, returning them to the library. That exhibit started in September and runs through " +
  "Oct. 10.";

// The real page as HTML, for the production-path test below — including
// this outlet's own real trailing "You May Also Be Interested In" teaser
// block (fabricated-but-realistic unrelated snippets, standing in for the
// real ones, since only the STRUCTURE of the boilerplate matters here) with
// its own unrelated venue/date, proving trimTrailingBoilerplate keeps it
// out of extraction on the full real pipeline, not just in isolation.
const BACKYARD_HTML =
  "<html><body><article><h1>" + BACKYARD_HEADLINE + "</h1><p>" +
  BACKYARD_BODY.replace(/"/g, "&quot;") +
  "</p></article>" +
  "<aside><h2>You May Also Be Interested In</h2>" +
  "<p>Rock out and run for those battling breast cancer: the 5K will return to Twelve Mile Crossing at " +
  "Fountain Walk on Oct. 18 to raise funds for local patients.</p>" +
  "<p>Road closure planned for Woodward Avenue near Main Street starting Oct. 3 for resurfacing work.</p>" +
  "</aside></body></html>";

function run() {
  const {
    extractTitle, extractVenue, extractDates, extractEventDates, extractCategory, extractDescriptionSentence,
    ...
    trimTrailingBoilerplate, linkPressCoverageQueue,
  } = freshLib();
  ...
  // --- 14. [RESOLVE EMBEDDED FUTURE EVENTS, 2026-09-25] extractTitle: a
  //     formal event name introduced only in the BODY via an explicit
  //     "titled/called/named/known as <quote>" construction -- the real
  //     Backyard and Beyond article, where neither the headline nor a
  //     quoted-headline phrase ever names the event. Must not fire on an
  //     earlier, unrelated quoted remark in the same body -- only the
  //     explicit naming verb anchors it. ---
  {
    assert.strictEqual(
      extractTitle(BACKYARD_HEADLINE, BACKYARD_BODY),
      "Backyard and Beyond",
      "must find the event name introduced in the body via \"titled '...'\", even with no formal title anywhere in the headline"
    );
    assert.strictEqual(
      extractTitle(
        "Fundraiser planned",
        "Organizer Jane Smith said, \"We are thrilled to help.\" The event, titled \"Community Giving Day,\" will run Oct. 4."
      ),
      "Community Giving Day",
      "an earlier unrelated quoted remark must not be mistaken for the title -- only the explicit titled/called/named/known-as construction counts"
    );
    assert.strictEqual(
      extractTitle("Fundraiser planned", "Jane Smith, who goes by the nickname \"Cakes,\" is organizing."),
      null,
      "a quoted nickname with no naming-verb anchor must never be mistaken for an event title"
    );
  }
  console.log("PASS: [RESOLVE EMBEDDED FUTURE EVENTS] extractTitle finds a formal event name introduced only in the body via an explicit titled/called/named/known-as construction, never a blind first-quote match");

  // --- 15. [RESOLVE EMBEDDED FUTURE EVENTS] extractEventDates: the real
  //     Backyard and Beyond article names its own future event date (Nov.
  //     9) alongside its own submission deadline (Oct. 10, stated twice)
  //     AND an unrelated other exhibit's own, coincidentally identical
  //     Oct. 10 date -- extractEventDates must keep only the real event
  //     date. extractDates() itself (unfiltered) is proven to still return
  //     all three raw dates -- this is disambiguation layered on top, not
  //     a change to extractDates() itself. ---
  {
    assert.deepStrictEqual(
      extractDates(BACKYARD_BODY, "2026-08-20T14:00:00+00:00"),
      ["2026-10-10", "2026-11-09"],
      "extractDates() alone still returns every date in the article -- proving the disambiguation lives in extractEventDates, not a change to extractDates() itself"
    );
    assert.deepStrictEqual(
      extractEventDates(BACKYARD_BODY, "2026-08-20T14:00:00+00:00", "Backyard and Beyond"),
      ["2026-11-09"],
      "must keep the real event date (Nov. 9, cued by \"goes live\") while excluding both nearby submission-deadline mentions of Oct. 10 (cued by \"deadline\") and the unrelated Tiny Art Show's own Oct. 10, well outside the title-anchored search window"
    );
    // No title -> no window to anchor -> falls back to extractDates()'s own
    // full, unfiltered result (never MORE permissive than extractDates()).
    assert.deepStrictEqual(
      extractEventDates(BACKYARD_BODY, "2026-08-20T14:00:00+00:00", null),
      ["2026-10-10", "2026-11-09"],
      "with no title to anchor a window, extractEventDates must fall back to extractDates()'s own full result, never invent a narrower answer without a real anchor"
    );
  }
  console.log("PASS: [RESOLVE EMBEDDED FUTURE EVENTS] extractEventDates disambiguates the real event date from a nearby submission deadline and an unrelated other event's coincidentally identical date");

  // --- 16. [RESOLVE EMBEDDED FUTURE EVENTS] trimTrailingBoilerplate: a
  //     synthetic, minimal proof that this outlet's real trailing "You May
  //     Also Be Interested In" teaser block -- which carries an UNRELATED
  //     other article's own address/venue -- must be discarded before any
  //     extractor ever sees it, not worked around case by case. ---
  {
    const untrimmed =
      "Announcing the Fall Bake Sale this Saturday at the community center. You May Also Be Interested In: " +
      "Join us at 20000 Woodward, Ferndale for the Fall Preview Night.";
    assert.deepStrictEqual(
      extractStreetAddress(untrimmed),
      { address: "20000 Woodward", city: "Ferndale" },
      "sanity check -- WITHOUT trimming, the boilerplate teaser's own address would otherwise leak through and be mistaken for this article's own location"
    );
    assert.strictEqual(
      extractStreetAddress(trimTrailingBoilerplate(untrimmed)),
      null,
      "trimming the trailing boilerplate block removes an unrelated OTHER article's own address before extraction ever sees it -- defense-in-depth stripHtml() alone can't provide"
    );
    assert.strictEqual(
      trimTrailingBoilerplate("An ordinary article with no boilerplate block at all."),
      "An ordinary article with no boilerplate block at all.",
      "an article with no recognized boilerplate heading must be returned unchanged, never truncated on a guess"
    );
  }
  console.log("PASS: [RESOLVE EMBEDDED FUTURE EVENTS] trimTrailingBoilerplate discards a real trailing teaser block's unrelated address/venue before any extractor sees it, and leaves an ordinary article untouched");
...
async function runProductionPathRegressionTests() {
  ...
  // --- 20. [RESOLVE EMBEDDED FUTURE EVENTS, 2026-09-25] End-to-end,
  //     production-path proof: the real "Photos sought for exhibit
  //     celebrating America's 250th birthday" article -- Jody's own
  //     override of an earlier "not a fit" judgment, with the explicit
  //     evidence that it embeds a real, confidently-describable future
  //     event ("Backyard and Beyond," opening Nov. 9 at the Madison
  //     Heights Public Library) -- now auto-creates via the REAL default
  //     wiring (nothing overridden, same PostgREST-faithful mock as tests
  //     16/19), with the CORRECT date (Nov. 9, never the Oct. 10
  //     submission deadline mentioned three times, never the unrelated
  //     Tiny Art Show's own Oct. 10), the real venue, and no invented end
  //     date/time/price. Also proves the real trailing "You May Also Be
  //     Interested In" teaser block this outlet's own template appends --
  //     carrying an unrelated OTHER article's own venue/date -- never
  //     leaks into the created event. ---
  {
    const backyardRow = {
      id: "article-backyard-prod",
      title: BACKYARD_HEADLINE,
      excerpt: "",
      url: "https://example.com/backyard-prod",
    ...
    const fetchFn = async (url, opts = {}) => {
      const method = opts.method || "GET";
      if (method === "GET" && url.includes("/editorial_articles")) {
        return { ok: true, status: 200, json: async () => [backyardRow] };
      }
      if (method === "GET" && url.includes("/events?status=")) {
        return { ok: true, status: 200, json: async () => [] };
      }
      ...
      if (method === "GET" && url === backyardRow.url) {
        return { ok: true, status: 200, text: async () => BACKYARD_HTML };
      }
      if (method === "POST" || method === "PATCH") {
        const contentType = opts.headers && opts.headers["Content-Type"];
        writeAttempts.push({ url, method, contentType });
        ...
        if (url.includes("/events") && !url.includes("/editorial_article_events") && method === "POST") {
          ...
          return { ok: true, status: 201, json: async () => [{ id: "evt-backyard-prod", ...createdBody }] };
        }
        return { ok: true, status: 204, json: async () => ({}) };
      }
      throw new Error(`unexpected fetch in Backyard and Beyond production-path regression test: ${method} ${url}`);
    };
    global.fetch = fetchFn;
    ...
    const counts = await realOrchestrator({
      ...
      SUPABASE_SERVICE_ROLE_KEY: "test-key",
      logger: silentLogger,
      nowIso: "2026-08-20", // "today" for this test -- well before the real Nov. 9 event
      repairGenericMetadataFn: async () => ({ written: 0 }),
      // Real default sbHeaders/createEvent/applyLink/fetchArticleText
      // wiring throughout (nothing overridden) -- exercises the same
      // trailing-boilerplate trim, title/date disambiguation, and
      // Content-Type header the real deployed Auto-Link path depends on.
    });

    assert.strictEqual(counts.autoCreated, 1, "an article primarily framed as a photo submission call must still resolve its embedded future event automatically");
    assert.strictEqual(counts.stillHuman, 0);
    assert.ok(createdBody, "the real default createEvent must have actually POSTed a row");
    assert.strictEqual(createdBody.title, "Backyard and Beyond");
    assert.strictEqual(createdBody.venue_name_raw, "Madison Heights Public Library", "must find the real venue named in the article's own caption, never something leaked from the trailing teaser block");
    assert.strictEqual(createdBody.category, "visual");
    assert.strictEqual(createdBody.start_date, "2026-11-09", "must be the event's own real opening date -- never the Oct. 10 submission deadline mentioned three times in the same article, and never the unrelated Tiny Art Show's own Oct. 10");
    assert.strictEqual(createdBody.end_date, null, "must never infer an end date that the article itself never stated");
    assert.ok(!("time_display" in createdBody) && !("price" in createdBody), "must never invent a time or a price -- neither was ever stated in the article");
    assert.ok(writeAttempts.length > 0);
    assert.ok(writeAttempts.every((a) => a.contentType === "application/json"));
  }
  console.log("PASS: [RESOLVE EMBEDDED FUTURE EVENTS] the real 'Photos sought...' article now auto-creates 'Backyard and Beyond' end to end -- correct future date, real venue, no fabricated end date/time/price, and the trailing boilerplate teaser block never leaks in");
}
