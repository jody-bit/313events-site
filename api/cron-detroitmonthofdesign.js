...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // the map, defaulting only those rows to DEFAULT_STATUS below). That
    // silently resets moderation state exactly like the unchunked D7 bug
    // did, just scoped to a chunk instead of the whole run. The shared
    // helper does its own <=100-id chunking internally and fails the WHOLE
    // lookup -- zero event writes, HTTP 502 -- if any chunk fails, rather
    // than defaulting any row's status. See api/_lib/status-lookup.js.
